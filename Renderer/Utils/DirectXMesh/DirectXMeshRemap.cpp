...
#pragma warning( disable : 6101 )
    ...
    template<class in_index_t, class out_index_t>
    HRESULT ReorderFaces(
        _In_reads_(nFaces * 3) const in_index_t* ibin, _In_ size_t nFaces,
        _In_reads_opt_(nFaces * 3) const uint32_t* adjin,
        _In_reads_(nFaces) const uint32_t* faceRemap,
        _Out_writes_(nFaces * 3) out_index_t* ibout,
        _Out_writes_opt_(nFaces * 3) uint32_t* adjout) noexcept
    {
        assert(ibin != nullptr && faceRemap != nullptr && ibout != nullptr);
        _Analysis_assume_(ibin != nullptr && faceRemap != nullptr && ibout != nullptr);
...
HRESULT __cdecl DirectX::ReorderIB(const uint32_t* ibin, size_t nFaces, const uint32_t* faceRemap, uint16_t* ibout) noexcept
{
    if (!ibin || !nFaces || !faceRemap || !ibout)
        return E_INVALIDARG;
    ...
    return ReorderFaces<uint32_t ,uint16_t>(ibin, nFaces, nullptr, faceRemap, ibout, nullptr);
    return E_NOTIMPL;
}
