...
namespace
{
    ...
    void KbmInitialize()
    {
        ...
        if (FAILED(s_Keyboard->SetCooperativeLevel(Graphics::ghWnd, DISCL_FOREGROUND | DISCL_NONEXCLUSIVE)))
        ...
        if (FAILED(s_Mouse->SetCooperativeLevel(Graphics::ghWnd, DISCL_FOREGROUND | DISCL_EXCLUSIVE)))
    ...
    void KbmUpdate()
    {
        ...
        if (foreground != Graphics::ghWnd // wouldn't be able to acquire
            || !visible)
        {
            KbmZeroInputs();
        }
